"""Real-time CNN posture inference with optional ESP32 feedback.

The CNN model is loaded from a local Keras artifact. The scaler and label map are
joblib files, so only use artifacts you trained yourself or explicitly trust.
"""
...
def require_artifacts(paths):
    ...
    if missing:
        ...
        for path in missing:
            print(f"  - {path}")
        print("Train local artifacts with 2_train_cnn.py before running inference.")
        sys.exit(1)


def main():
    require_artifacts([CNN_MODEL_PATH, SCALER_PATH, LABEL_MAP_PATH])
    print("Loading local model artifacts. Only proceed if you trust or trained these files.")
    clf = tf.keras.models.load_model(CNN_MODEL_PATH)
    scaler = joblib.load(SCALER_PATH)
    label_map = joblib.load(LABEL_MAP_PATH)

    mp_pose = mp.solutions.pose
    mp_drawing = mp.solutions.drawing_utils
    pose = mp_pose.Pose(static_image_mode=False, model_complexity=1, smooth_landmarks=True, min_detection_confidence=0.6, min_tracking_confidence=0.6)
    ...
    print("Running CNN inference. Press C to calibrate while sitting tall. Press Q to quit.")
    ...
    try:
        while True:
            ...
            cv2.imshow("Project Angelina - CNN Inference", frame)
